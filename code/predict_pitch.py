...
def wav_to_tfrd(wav_paths, predict_dir):
    p = Pool(16)
    generate_tfrd = partial(generate_test_tfrd, predict_dir=predict_dir)
    result = p.map_async(generate_tfrd, wav_paths)
    result.get()
    p.close()
    p.join()
...
def main(args):
    ...
    audio_files = [filename for filename in audio_files if
                   filename.replace(suffix, '.pitch_raw') not in os.listdir(args.save_raw_dir)]
